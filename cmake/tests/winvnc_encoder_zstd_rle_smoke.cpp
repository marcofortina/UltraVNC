...
int main()
{
    ...
    winvnc_test_expect(encoder.SetRemoteFormat(format), "ZSTDRLE remote format should initialize");
    winvnc_test_expect(encoder.SetLocalFormat(format, 8, 8), "ZSTDRLE local format should initialize");
    ...
    winvnc_test_expect(encoded > sz_rfbFramebufferUpdateRectHeader + sz_rfbZRLEHeader, "ZSTDRLE encoder should emit compressed payload");
    const auto *header = reinterpret_cast<const rfbFramebufferUpdateRectHeader *>(dest.data());
    winvnc_test_expect(winvnc_test_host32(header->encoding) == rfbEncodingZSTDRLE, "unexpected ZSTDRLE encoding");
    const auto *zrle = reinterpret_cast<const rfbZRLEHeader *>(dest.data() + sz_rfbFramebufferUpdateRectHeader);
    winvnc_test_expect(winvnc_test_host32(zrle->length) == encoded - sz_rfbFramebufferUpdateRectHeader - sz_rfbZRLEHeader, "unexpected ZSTDRLE payload length");
    return 0;
}
