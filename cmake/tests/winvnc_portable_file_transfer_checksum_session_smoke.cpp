...
int main()
{
    const std::string root = std::string("/tmp/uvnc-ft-checksum-session-") + std::to_string(getpid());
    ...
    SendFileTransferMessage(clientSocket, rfbFileChecksums, 0, "file.bin");
    ...
    assert(response.contentType == rfbFileChecksums);
    assert(Swap32IfLE(response.size) == 1);
    std::string payload(Swap32IfLE(response.length), '\0');
    assert(clientSocket.ReadExact(&payload[0], payload.size()));
    assert(payload.find("0:6:") != std::string::npos);
