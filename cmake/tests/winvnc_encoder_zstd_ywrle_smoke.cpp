...
int main()
{
    ...
    winvnc_test_expect(encoder.SetRemoteFormat(format), "ZSTDYWRLE remote format should initialize");
    winvnc_test_expect(encoder.SetLocalFormat(format, 8, 8), "ZSTDYWRLE local format should initialize");
    encoder.m_use_zywrle = true;
    encoder.set_use_zstd(true);

    const std::uint32_t pixel = 0x20304050;
    ...
    winvnc_test_expect(encoded > sz_rfbFramebufferUpdateRectHeader + sz_rfbZRLEHeader, "ZSTDYWRLE encoder should emit compressed payload");
    const auto *header = reinterpret_cast<const rfbFramebufferUpdateRectHeader *>(dest.data());
    winvnc_test_expect(winvnc_test_host32(header->encoding) == rfbEncodingZSTDYWRLE, "unexpected ZSTDYWRLE encoding");
    const auto *zrle = reinterpret_cast<const rfbZRLEHeader *>(dest.data() + sz_rfbFramebufferUpdateRectHeader);
    winvnc_test_expect(winvnc_test_host32(zrle->length) == encoded - sz_rfbFramebufferUpdateRectHeader - sz_rfbZRLEHeader, "unexpected ZSTDYWRLE payload length");
    return 0;
}
