...
int main()
{
    const std::string root = std::string("/tmp/uvnc-ft-recursive-limits-") + std::to_string(getpid());
    ...
    config.SetFileTransferRecursiveMaxDepth(1);
    config.SetFileTransferRecursiveMaxEntries(8);
    ...
    assert(items.count("dir/nested") == 0);
    assert(items.count("dir/nested/file.txt") == 0);
