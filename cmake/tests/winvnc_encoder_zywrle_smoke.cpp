...
int main()
{
    ...
    winvnc_test_expect(encoder.SetRemoteFormat(format), "ZYWRLE remote format should initialize");
    winvnc_test_expect(encoder.SetLocalFormat(format, 8, 8), "ZYWRLE local format should initialize");
    ...
    winvnc_test_expect(encoded > sz_rfbFramebufferUpdateRectHeader + sz_rfbZRLEHeader, "ZYWRLE encoder should emit compressed payload");
    const auto *header = reinterpret_cast<const rfbFramebufferUpdateRectHeader *>(dest.data());
    winvnc_test_expect(winvnc_test_host32(header->encoding) == rfbEncodingZYWRLE, "unexpected ZYWRLE encoding");
    const auto *zrle = reinterpret_cast<const rfbZRLEHeader *>(dest.data() + sz_rfbFramebufferUpdateRectHeader);
    winvnc_test_expect(winvnc_test_host32(zrle->length) == encoded - sz_rfbFramebufferUpdateRectHeader - sz_rfbZRLEHeader, "unexpected ZYWRLE payload length");
    return 0;
}
