...
if [[ -n "${INSTALL_PREFIX}" ]]; then
    ...
    "${INSTALL_PREFIX}/bin/uvnc_winvnc_memory_server" --smoke-update-test --pattern checker --width 64 --height 32 --name installed-update-smoke
    "${INSTALL_PREFIX}/bin/uvnc_winvnc_memory_server" --smoke-multi-update-test --max-updates 3 --pattern gradient-x --width 64 --height 32 --name installed-multi-update-smoke
fi
